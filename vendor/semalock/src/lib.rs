...
#[inline]
fn sem_timedwait_compat( sem: *mut libc::sem_t, sem_timeout: &libc::timespec ) -> i32 {
    #[cfg(any(
        target_os = "linux",
        target_os = "android",
        target_os = "freebsd",
    ...
    {
        return unsafe { libc::sem_timedwait( sem, sem_timeout ) };
    }
    ...
    {
        let deadline_ns = (sem_timeout.tv_sec as i128) * 1_000_000_000i128 + (sem_timeout.tv_nsec as i128);
        loop {
            ...
            let e = errno::errno();
            if e.0 != libc::EAGAIN && e.0 != libc::EINTR {
                return -1;
            }
            ...
            let now = SystemTime::now()
                ...
                .unwrap_or_default();
            let now_ns = (now.as_secs() as i128) * 1_000_000_000i128 + (now.subsec_nanos() as i128);
            if now_ns >= deadline_ns {
                errno::set_errno( errno::Errno( libc::ETIMEDOUT ) );
                return -1;
            }

            // Sleep briefly to avoid busy spinning.
            let sleep_ts = libc::timespec { tv_sec: 0, tv_nsec: 1_000_000 };
            unsafe {
...
impl Semalock {
    ...
    fn acquire(&self) -> Result<(), SemalockError> {
        loop {
            ...
            let call_status = sem_timedwait_compat(self.sem, &sem_timeout);
